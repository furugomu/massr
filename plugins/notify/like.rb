	...
	module Plugin::Notify
		class Like
			...
			def self.delete(id, name, me)
				if SETTINGS[id].delete == 'owner' && name == me.massr_id
					likes(id).delete(name)
				elsif SETTINGS[id].delete == 'any'
					likes(id).delete(name)
				end
				return nil
			end
		...
		private
			...
			def icons
				LIKES[@id].map{|user, (http, https)|
					case @delete
					when "any"
						%Q|<a href="#" class="#{@id}-delete"><img class="massr-icon-mini" src="#{https}" alt="#{user}" title="delete"></a>|
					else # "owner"
